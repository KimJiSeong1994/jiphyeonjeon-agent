...
## 요약
현재 실노출은 **브랜드/GitHub 직검색 수준**. "research/MCP 키워드"는 **트리거 2개(punkpeye PR 머지 · Glama 색인)** 가 켜지면 노출 시작된다. 논문·비교·한국·브랜드 키워드는 각각 별도 작업이 선행돼야 한다.
...
|---|---|---|---|
| **punkpeye awesome PR #8291 머지** | 🟡 OPEN(머지 대기) | `best MCP server for research` · `academic MCP server` · `arxiv MCP` · `research MCP` — LLM이 인용하는 최대 awesome 코퍼스 진입 | `gh pr view 8291 --repo punkpeye/awesome-mcp-servers` |
...
## 점검 루틴 (월간 권장)
1. punkpeye PR #8291 머지 여부 → 머지되면 해당 행을 🟢로 이동, 키워드군을 "오늘 라이브"로 승격.
2. Glama 색인 여부(위 URL).
