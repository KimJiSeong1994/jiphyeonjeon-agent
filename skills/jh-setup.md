...
2. **집현전 백엔드 기동 여부**: `curl -sS http://localhost:8000/health` (로컬) 또는 `curl -sS https://jiphyeonjeon.kr/health` (프로덕션). 미기동이면 백엔드 실행 안내 후 계속 진행 여부 질의.
3. **CLI 가용성**: `uv --version`, `claude --version` 둘 다 PATH 에 있는지 체크. 없으면 각각 [uv 설치](https://docs.astral.sh/uv/) / Claude Code 설치 안내.

## 실행 순서

1. **자격증명 수집 (사용자가 직접 터미널 실행)**
...
3. **대안 2: 프로덕션 서버 연결**
   ```bash
   JIPHYEONJEON_TOKEN=<jwt> JIPHYEONJEON_BASE_URL=https://jiphyeonjeon.kr \
